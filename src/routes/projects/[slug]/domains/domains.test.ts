...
describe('domains load', () => {
	...
	it('computes defaultSubdomain from a subdomain-mode domain_config', async () => {
		vi.mocked(getSetting).mockResolvedValueOnce(
			JSON.stringify({ mode: 'subdomain', baseDomain: 'risved.example.com' })
		);
		dbAny.__limitMock.mockResolvedValueOnce([
			{ id: 'proj-1', name: 'Test', slug: 'test', port: 3001 }
		]);
		...
		expect(result.defaultSubdomain).toBe('test.risved.example.com');
	});

	it('leaves defaultSubdomain null when domain_config mode is ip', async () => {
		vi.mocked(getSetting).mockResolvedValueOnce(
			JSON.stringify({ mode: 'ip', baseDomain: 'risved.example.com' })
		);
		dbAny.__limitMock.mockResolvedValueOnce([
			{ id: 'proj-1', name: 'Test', slug: 'test', port: 3001 }
		]);
...
describe('domains actions', () => {
	...
	it('add returns 409 when the domain already exists', async () => {
		dbAny.__limitMock
	...
	it('verify checks both A and AAAA records when the server has an IPv6 address', async () => {
		...
		dbAny.__limitMock
			.mockResolvedValueOnce([{ id: 'proj-1' }])
			.mockResolvedValueOnce([
				{ id: 'dom-1', hostname: 'app.example.com', sslStatus: 'pending', verifiedAt: null, projectId: 'proj-1' }
			]);
