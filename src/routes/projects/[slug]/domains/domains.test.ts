...
describe('domains load', () => {
	...
	it('computes defaultSubdomain from a custom-domain config', async () => {
		const { getSetting } = await import('$lib/server/settings');
		vi.mocked(getSetting).mockResolvedValueOnce(
			JSON.stringify({ mode: 'custom', baseDomain: 'apps.example.com' })
		);
		dbAny.__limitMock.mockResolvedValueOnce([
			{ id: 'proj-1', name: 'Test', slug: 'test', port: 3001 }
		]);
		...
		expect(result.defaultSubdomain).toBe('test.apps.example.com');
	});

	it('ignores baseDomain when mode is ip', async () => {
		const { getSetting } = await import('$lib/server/settings');
		vi.mocked(getSetting).mockResolvedValueOnce(
			JSON.stringify({ mode: 'ip', baseDomain: 'apps.example.com' })
		);
		dbAny.__limitMock.mockResolvedValueOnce([
			{ id: 'proj-1', name: 'Test', slug: 'test', port: 3001 }
		]);
...
describe('domains actions', () => {
	...
	it('add returns 404 when project not found', async () => {
		dbAny.__limitMock.mockResolvedValueOnce([]); // project lookup
