...
describe('delivery detail load', () => {
	...
	it('falls back to the raw payload string when it is not valid JSON', async () => {
		dbAny.__limitMock
			.mockResolvedValueOnce([{ id: 'proj-1', name: 'App', slug: 'app' }])
			.mockResolvedValueOnce([
				{
		...
		const result = (await load({
			params: { slug: 'app', did: 'del-1' }
		} as Parameters<typeof load>[0])) as { delivery: { payload: unknown } };

		expect(result.delivery.payload).toBe('not-json-at-all');
	});
});
