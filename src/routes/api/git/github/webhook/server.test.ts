...
const VALID_BODY = { connectionId: 'conn-1', projectId: 'proj-1', owner: 'alice', repo: 'myrepo' }
...
describe('POST /api/git/github/webhook', () => {
	...
	it('creates webhook and returns success', async () => {
		...
		mockCreateWebhook.mockResolvedValue({ id: 77 })

		const { POST } = await import('./+server')
		const resp = await POST(makeEvent(VALID_BODY) as never)
		expect(resp.status).toBe(200)
		const data = await resp.json()
		expect(data).toMatchObject({ success: true, webhookId: 77 })
	})

	it('uses hostname setting when available', async () => {
		mockGetSetting.mockResolvedValue('myapp.com')
		mockDb.select
			.mockReturnValueOnce(makeSelectChain([{ id: 'conn-1', accessToken: 'enc' }]))
			.mockReturnValueOnce(makeSelectChain([{ id: 'proj-1', webhookSecret: 'sec' }]))
		...
		expect(mockCreateWebhook).toHaveBeenCalledWith(
			expect.objectContaining({ webhookUrl: 'https://myapp.com/api/webhooks/proj-1' })
		)
	})
})
