...
const REPO = {
	...
	private: false,
	default_branch: 'main',
	html_url: 'https://git.example.com/alice/repo',
...
describe('GET /api/git/forgejo/repos', () => {
	it('returns 400 when connectionId is missing', async () => {
		const { GET } = await import('./+server')
		const resp = await GET(makeEvent({ instanceUrl: 'https://git.example.com' }) as never)
	...
	it('returns 404 when connection not found', async () => {
		...
		const resp = await GET(
			makeEvent({ connectionId: 'conn-1', instanceUrl: 'https://git.example.com' }) as never
		)
