...
/**
 * POST /api/git/forgejo/webhook — auto-configure webhook on a Forgejo/Gitea repo.
 * Body: { connectionId, projectId, owner, repo, instanceUrl }
 */
export const POST: RequestHandler = async (event) => {
	...
	const { connectionId, projectId, owner, repo, instanceUrl } = body as Record<string, string>;

	if (!connectionId || !projectId || !owner || !repo || !instanceUrl) {
		return jsonError(400, 'connectionId, projectId, owner, repo, and instanceUrl are required');
	}
