...
describe('demo/better-auth/login load', () => {
	it('redirects to /demo/better-auth when already logged in', async () => {
...
describe('demo/better-auth/login signInSocial action', () => {
	...
	it('redirects to provider url on success', async () => {
		...
		expect(auth.api.signInSocial).toHaveBeenCalledWith(
			expect.objectContaining({
				body: { provider: 'github', callbackURL: '/demo/better-auth' }
			})
		);
	});

	it('defaults provider to github and callbackURL to /demo/better-auth', async () => {
		vi.mocked(auth.api.signInSocial).mockResolvedValue({
			url: 'https://github.com/login/oauth/authorize'
		} as unknown as Awaited<ReturnType<typeof auth.api.signInSocial>>);

		await expect(actions.signInSocial(makeActionEvent({}))).rejects.toMatchObject({
			status: 302
		});
	...
	it('fails when the provider returns no url', async () => {
		vi.mocked(auth.api.signInSocial).mockResolvedValue(
