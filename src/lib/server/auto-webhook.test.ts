...
describe('registerWebhook', () => {
	...
	it('skips webhook for a valid URL on a non-github host', async () => {
		setupSelect([{ accessToken: 'enc-tok', provider: 'github' }]);
		await registerWebhook({ ...BASE_OPTS, repoUrl: 'https://gitlab.com/owner/repo' });
		expect(mockCreateWebhook).not.toHaveBeenCalled();
	});

	it('skips webhook when the github URL path is missing a repo segment', async () => {
		setupSelect([{ accessToken: 'enc-tok', provider: 'github' }]);
		await registerWebhook({ ...BASE_OPTS, repoUrl: 'https://github.com/owner' });
		expect(mockCreateWebhook).not.toHaveBeenCalled();
	});
