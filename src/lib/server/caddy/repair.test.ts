...
function mockCaddy(
	overrides: Partial<Record<'addRoute' | 'addRedirectRoute', ReturnType<typeof vi.fn>>> = {}
) {
	return {
		addRoute: overrides.addRoute ?? vi.fn().mockResolvedValue({ success: true }),
		addRedirectRoute: overrides.addRedirectRoute ?? vi.fn().mockResolvedValue({ success: true })
	};
}
...
describe('repairDomainRoute', () => {
	...
	it('skips the redirect route when the hostname already has a www prefix', async () => {
		...
		expect(caddy.addRoute).toHaveBeenCalledWith({ hostname: 'www.example.com', port: 4001 });
