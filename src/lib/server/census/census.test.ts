...
describe('CensusReporter', () => {
	...
	beforeEach(() => {
		...
		delete env.RISVED_CENSUS_ENDPOINT;
	...
	describe('getInstanceId', () => {
		...
		it('seeds the Cloud instance UUID from env', async () => {
	...
	describe('ping', () => {
		it('uses the configured Cloud census endpoint by default', async () => {
			env.RISVED_CENSUS_ENDPOINT = 'https://cloud.example.com/api/census';
			reporter = new CensusReporter();
			const fetchSpy = vi
			...
			expect(url).toBe('https://cloud.example.com/api/census');
		});
